...
$file = trim($argv[1]);
if(strlen($file)==0){
    print("zadny vstupni soubor ?");
    exit(1);
}

echo "vstupni soubor: $file\n";

$dom = new DOMDocument;
$dom->load($file);
if (!$dom) {
    echo "chyba pri parsovani dokumentu $file\n";
    exit(1);
}
    ...
    foreach ($persons as $person) {
        ...
        $persnr = $person->getAttribute('persno');
	$profile = $person->getAttribute('profile');
	$opentime = $person->getAttribute('opentime');
	
	echo "$persnr\t$profile\t$opentime\n";
    }
