...
require_once '../db.php';
    ...
    $bearbZeit = intval($kdInfoArray[0]['bearbeitung_tage']);
    $sollExDateTime = date('Y-m-d H:i',  strtotime($sollImDateTime)+$bearbZeit*24*60*60);
    
    //standartni zielort_id
    $standard_zielort_id = $apl->getStandardZielortId($kunde);
    // vytvorit hlavicku zakazky
    $insertId = $apl->createNewImport($imNr,$kunde,$minpreis,$aufdatDateTime,$sollImDateTime,$waehrKz,$bemerkung,$bestellNr,$sollExDateTime,$standard_zielort_id);
    $insertId = 1;
...
//vytvorit pozice v zakazce, v pripade ze pozaduji nenulovy pocet kusu planovaciho dilu
    ...
    $kundeBoxId1="tag_".substr($sollExDateTime,0,10)."_".$kunde;
