...
$smarty = new Smarty;
	...
	if(isset($_GET['auftragsnr']))
	{
		...
		$sql="select id_dauftr,Teil,`pos-pal-nr` as  pos_pal_nr,`Stück` as stk,Preis,`MehrArb-KZ` as mehrarb_kz,abgnr,KzGut,Termin,`auftragsnr-exp` as auftragsnr_exp,`pal-nr-exp` as pos_pal_nr_exp,`stk-exp` as stk_exp,fremdauftr,fremdpos  from dauftr where (auftragsnr='".$_GET['auftragsnr']."') order by pos_pal_nr,abgnr";
		$res=mysql_query($sql);
