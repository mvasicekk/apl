...
$a = AplDB::getInstance();
	...
	$tat6_kdmin=trim($_GET['tat6_kdmin']);
        $amnr = trim($_GET['amnr']);
        $ausstk = intval(trim($_GET['ausstk']));
        $rueckstk = intval(trim($_GET['rueckstk']));
	...
	$vzkd_array = array();
        ...
        $bCopyVzAbyToVerb = $a->getCopyVzAbyToVerbFlag($persnr);
        if($auss_typ==4){
            $gesamtStk = $stk+$auss_stk;
        }
        else{
            $gesamtStk = $stk;
        }
		...
		for($i=0;$i<sizeof($tat_array);$i++)
		{
			if($vzaby_pro_stk!=0){
				$cast_verb = round($vz_array[$i]/$vzaby_pro_stk*$verb);
                                if($bCopyVzAbyToVerb){
                                    $cast_verb = $gesamtStk*$vz_array[$i];
                                }
                        }
			else
			{
	...
	$output .= "<auftragsnr_old>$auftragsnr</auftragsnr_old>";
        $output .= "<copyvzaby>$bCopyVzAbyToVerb</copyvzaby>";
	...
	for($i=0;$i<sizeof($insert_array);$i++)
	{
		...
		$output .= '</affected_rows>';
                ...
                // vlozim si dochazku do dzeittest ale jen pro prvni operaci
                if($i==0){
                    $verbminuten = $verb/60-$pause/60;
                    $pausestd = $pause/60;
                    $sqldzeit = "insert into dzeittest (persnr,datum,stunden,schicht,tat,anw_von,anw_bis,pause1,comp_user_accessuser) ";
                    $sqldzeit.= " values (".$persnr.",'".make_DB_datum($datum)."',".$verbminuten.",".$schicht.",'".$oe."','".make_DB_datetime($von, $datum)."','".make_DB_datetime($bis, $datum)."',".$pausestd.",'".$ident."')";
//                    mysql_query($sqldzeit);

                    // arbeitsmittelausgabe vlozim do tabulky
                    if(strlen($amnr)>0){
