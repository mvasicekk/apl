...
//                $sql="select `Name` as bezd,`Name` as bezt,abgnr,vzkd,vzaby from dauftr join `dtaetkz-abg` on `dtaetkz-abg`.`abg-nr`=dauftr.`abgnr` where ((`abgnr`='".$value."') and (`teil`='".$teil."') and (`pos-pal-nr`='".$pal."') and (auftragsnr='".$auftragsnr."') and (dauftr.abgnr<>3)) order by abgnr";
		$result=mysql_query($sql);
