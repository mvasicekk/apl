...
case 'tatigkeit': tatigkeit($_GET['auftr'],$_GET['pal'],$_GET['teil']);  break;
case 'pers': persNr($_GET['pers']); break;
case 'min': minuty($_GET['auftr'],$_GET['teil'],$_GET['tat'], $_GET['feld']); break;
...
function tatigkeit($auftr, $pal, $teil){
$sql = "select `AuftragsNr`, `pos-pal-nr`, `Teil`, `abgnr` from Dauftr where `AuftragsNr`='$auftr' and `pos-pal-nr`=$pal and `Teil`='$teil'";
$res = mysql_query($sql) or die(mysql_error());

  if(mysql_affected_rows()>0){
    $vypis ="naplnOperace('0";
    while($zaznam = mysql_fetch_array($res)){
    $vypis = $vypis.",".$zaznam["abgnr"];
    }
    $vypis = $vypis." ')";
...
function minuty($auftr, $teil, $tat, $feld){
$sql = "select `VzAby`, `VzKd`  from Dauftr where ((`auftragsnr`='$auftr') and (`teil`='$teil') and (`abgnr`='$tat'))";
$res = mysql_query($sql) or die(mysql_error());

if(mysql_affected_rows()>0){

    $zaznam = mysql_fetch_array($res);
    $vypis ="naplnMin('".$zaznam["VzAby"]."','".$zaznam["VzKd"]."',$feld)";
