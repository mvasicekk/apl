 ...
 */
function pageheader($pdfobjekt,$rgb,$vyskaradku,$last,$wd=10) {
	...
	//vzkd
        $obsah = "VzKd";
        $pdfobjekt->SetFont("FreeSans", "B", 7);
        $pdfobjekt->Cell($wd,$vyskaradku,$obsah,'TLB',0,'R',$fill);
        ...
        $obsah = "VzKd/VzAby";
        $pdfobjekt->SetFont("FreeSans", "B", 5);
        $pdfobjekt->Cell($wd,$vyskaradku,$obsah,'TLBR',1,'R',$fill);
 ...
 * @param <type> $childs
 */
function schicht_radek($ogFarbenArray,$pdfobjekt,$vyskaradku,$rgb,$childs,$wd=10){
...
//        print_r($ogRGBArray);
        ...
        if(!strcmp($obsah, 'Ox4') && ($pageOx4==0)){
            $pdfobjekt->AddPage();
            pageheader($pdfobjekt,array(255,255,200),5,0,$wd);
            $pageOx4 = 1;
            $pdfobjekt->SetFont("FreeSans", "", 7);
        }
...
function zapati_sestava($pdfobjekt,$vyskaradku,$rgb,$pole,$ende=TRUE,$wd){
...
function zapati_sestava_index($pdfobjekt,$vyskaradku,$rgb,$pole,$anwesend=TRUE,$wd=10){
        ...
        $pdfobjekt->SetFont("FreeSans", "", 7);
        $pdfobjekt->Cell($wd,$vyskaradku,$obsah,'TLBR',0,'R',$fill);
        ...
        $pdfobjekt->Cell(13+10+35,$vyskaradku,$obsah,'TLBR',0,'L',$fill);
                ...
                $pdfobjekt->Cell(5*$wd,$vyskaradku,'','0',0,'L',0);
        //a
        $obsah = number_format($pole['minuten_a']/$anwMinuten*100,1,',',' ');
        $pdfobjekt->SetFont("FreeSans", "", 7);
        $pdfobjekt->Cell($wd,$vyskaradku,$obsah,'TLB',0,'R',$fill);
...
function zapati_schichtgruppen($ogFarbenArray,$pdfobjekt,$vyskaradku,$rgb,$pole,$polegesamt,$poletext,$wd=10){
...
//            $pdfobjekt->Cell(0,$vyskaradku,"",'0',1,'R',$fill);
        ...
        zapati_sestava($pdfobjekt,4,array(230,230,255),$polegesamt,TRUE,$wd);
        zapati_sestava_index($pdfobjekt,4,array(230,230,255),$polegesamt,TRUE,$wd);
        zapati_sestava_index($pdfobjekt,4,array(230,230,255),$polegesamt,FALSE,$wd);
        
        // a jeste jednou s procentnim podilem z gesamt
        $decimals = 1;
        $pdfobjekt->Ln();
        pageheader($pdfobjekt,array(255,255,200),5,1,$wd);
        $summeProzent = array();
        foreach($gruppen as $gruppe) {
            $ogRGBArray = split(',', $ogFarbenArray[$gruppe]);
            $pdfobjekt->SetFillColor($ogRGBArray[0], $ogRGBArray[1], $ogRGBArray[2]);
 ...
 * @param <type> $monat
 */
function test_pageoverflow($pdfobjekt,$vysradku,$rgb,$last,$wd=10)
{
	...
	{
		$pdfobjekt->AddPage();
		pageheader($pdfobjekt,$rgb,$vysradku,$last,$wd);
	}
}
...
$pocetDetailSloupcu = 21;
$wd = ($pdf->getPageWidth()-(PDF_MARGIN_LEFT-10)-(PDF_MARGIN_RIGHT-10)-(12+13+35))/$pocetDetailSloupcu;

// prvni stranka
$pdf->AddPage();
pageheader($pdf,array(255,255,200),5,0,$wd);
...
foreach($schichten as $schicht){
    ...
    if(testValues($schichtChilds)){
        test_pageoverflow($pdf, 5, array(255,255,200),0,$wd);
        schicht_radek($ogFarbenArray[$schichtgruppe],$pdf,4,array(255,255,255),$schichtChilds,$wd);
    }
}
...
test_pageoverflow($pdf, 5, array(255,255,200),0,$wd);
zapati_sestava($pdf,4,array(230,230,255),$sum_zapati_sestava_array,FALSE,$wd);

$pdf->AddPage();
pageheader($pdf,array(255,255,200),5,1,$wd);
//$pdf->Ln();
//zapati_sestava($pdf,4,array(230,230,255),$sum_zapati_sestava_array);
zapati_schichtgruppen($ogFarbenArray,$pdf,4,array(230,255,255),$sum_zapati_schichtgruppe_array,$sum_zapati_sestava_array,$text_zapati_schichtgruppe_array,$wd);
