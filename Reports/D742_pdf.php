...
if($typ=='nur Summe Teil') {
    $bMitSumme=TRUE;
    $bNurSumme=TRUE;
}

require_once('D742_xml.php');
...
//"waehrung"
	...
	$pdfobjekt->SetFont("FreeSans", "B", 8);
        ...
        $pdfobjekt->Cell(
                    $cells['teilnr']['sirka']
                    ,$vyskaradku,  getValueForNode($childNodes,'teilnr'),'TB',0,'L',1);

                $pdfobjekt->Cell(
                    $cells['teilbez']['sirka']
                    ,$vyskaradku,  getValueForNode($childNodes,'teilbez'),'TB',0,'L',1);
        ...
        $obsah = getValueForNode($childNodes,'wahr');
      	$pdfobjekt->Cell(
                    $cells['waehrung']['sirka']
                    ,$vyskaradku,$obsah,'TB',0,'L',1);
...
foreach ($rechnunge as $rechnung) {
    ...
    foreach ($fremdauftraege as $fremdauftrag) {
        ...
        nuluj_sumy_pole($sum_zapati_fremdpos_array);
        //test_pageoverflow($pdf,5,$cells_header);
            ...
            foreach ($teile as $teil) {
                ...
                foreach ($taetigkeiten as $taetigkeit) {
                    ...
                    foreach ($kusy as $kus) {
                        ...
                        $gutStk += intval(getValueForNode($kusChildNodes, 'gut_stk'));
                    }
                }
                ...
                foreach ($taetigkeiten as $taetigkeit) {
                    ...
                    foreach ($kusy as $kus) {
                        ...
                        if(!$bNurSumme) detaily($pdf, $cells, 5, array(255, 255, 255), $kusChildNodes);
...
//                    }
            ...
            if (test_pageoverflow_noheader($pdf, 1))
                pageheader($pdf, $cells_header, $rechnungChildNodes);
            zapati_fremdpos($pdf, 1, array(255, 255, 255), $fremdpositionChildNodes,$sum_zapati_fremdpos_array);
        }
    }
    // zapati rechnung
