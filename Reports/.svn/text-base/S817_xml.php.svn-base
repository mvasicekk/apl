...
$sql="SELECT dauftr.AuftragsNr, dauftr.Teil, dauftr.Stück, dauftr.`pos-pal-nr`, ";
$sql.=" dkopf.Gew, `gew`*`stück` AS vahacelkem, `brgew`*`stück` AS vahacelkembr, dauftr.`auftragsnr-exp`, ";
$sql.=" ".$pt_hotovosum.".gutstk, ".$pt_hotovosum.".auss, dauftr.KzGut, DATE_FORMAT(daufkopf.Aufdat,'%Y-%m-%d') as Aufdat, dauftr.Termin";
$sql.=" FROM dauftr ";
$sql.=" join daufkopf on daufkopf.auftragsnr=dauftr.auftragsnr";
$sql.=" join dkopf on dkopf.teil=dauftr.teil";
$sql.=" left join ".$pt_hotovosum." ON (dauftr.AuftragsNr = ".$pt_hotovosum.".AuftragsNr) AND (dauftr.Teil = ".$pt_hotovosum.".Teil) AND (dauftr.`pos-pal-nr` = ".$pt_hotovosum.".`pos-pal-nr`)";
$sql.=" WHERE (((dauftr.AuftragsNr) Between '".$auftragsnr_von."' And '".$auftragsnr_bis."') AND ((dauftr.Teil) Like '".$teil."') AND ((dauftr.`pos-pal-nr`)>0) AND ((dauftr.`auftragsnr-exp`) Is Null) and ($pt_hotovosum.`pos-pal-nr` is null) AND ((dauftr.KzGut)='G'))";
