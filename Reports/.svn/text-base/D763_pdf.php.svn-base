...
$sum_zapati_position_array = array(
								"betrag"=>0,
								);
global $sum_zapati_position_array;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function zapati_fremdpos($pdfobjekt,$vyskaradku,$rgb,$childNodes,$sumarray,$w)
{

	$pdfobjekt->SetFillColor($rgb[0],$rgb[1],$rgb[2],1);
	$pdfobjekt->SetFont("FreeSans", "B", 8);
	$pdfobjekt->Cell(20+45+45+12+10+20,$vyskaradku,"Summe Position",'B',0,'L',1);
    $obsah=number_format($sumarray['betrag'],2,',',' ');
    $pdfobjekt->Cell(15,$vyskaradku,$obsah,'TB',0,'R',1);
    $pdfobjekt->Cell(0,$vyskaradku,$w,'TB',1,'R',1);
}
...
function zapati_sestava($pdfobjekt,$vyskaradku,$rgb,$childNodes)
{
    ...
    // verwendungszweck
    $zweck=getValueForNode($childNodes,"verzweck");
...
function zahlavi_fremdposition($pdfobjekt,$vyskaradku,$rgb,$childNodes)
{
	$pdfobjekt->SetFillColor($rgb[0],$rgb[1],$rgb[2],1);
	$pdfobjekt->SetFont("FreeSans", "B", 8);
	$pdfobjekt->Cell(20,$vyskaradku,"Best.Nr.:",'0',0,'L',0);
	$fremdauftr = getValueForNode($childNodes,"fremdauftrnr");
	$pdfobjekt->Cell(45,$vyskaradku,$fremdauftr,'0',0,'L',0);
	$fremdpos = getValueForNode($childNodes,"fremdposnr");
	$pdfobjekt->Cell(45,$vyskaradku,"Pos.:".$fremdpos,'0',1,'L',0);
}
...
//$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, "D743 Rechnung", $params);
...
foreach($rechnunge as $rechnung)
{
    ...
    $z1 = getValueForNode($rechnungChildNodes,"fusszeile1");
    $z2 = getValueForNode($rechnungChildNodes,"fusszeile2");
    $z3 = getValueForNode($rechnungChildNodes,"fusszeile3");
    $wahr = getValueForNode($rechnungChildNodes,"wahr");
	...
	foreach($fremdauftraege as $fremdauftrag)
	{
		...
		foreach($fremdpositionen as $fremdposition)
		{
			...
			nuluj_sumy_pole($sum_zapati_position_array);
			...
			foreach($teile as $teil)
			{
				...
				foreach($taetigkeiten as $taetigkeit)
				{
                    ...
                    foreach($kusy as $kus){
                        ...
                        //aktualizuju sumy pro zapati position
    					foreach($sum_zapati_position_array as $key=>$prvek)
    					{
    						$hodnota=getValueForNode($kusChildNodes,$key);
    						$sum_zapati_position_array[$key]+=$hodnota;
    					}
                    }
...
//					}
			...
			if(test_pageoverflow_noheader($pdf,5))
						pageheader($pdf,$cells_header,$rechnungChildNodes);
			zapati_fremdpos($pdf,5,array(255,255,255),$fremdpositionChildNodes,$sum_zapati_position_array,$wahr);
		}
	}
	// zapati rechnung
	if(test_pageoverflow_noheader($pdf,18))
			pageheader($pdf,$cells_header,$rechnungChildNodes);
	zapati_rechnung($pdf,6,array(255,255,255),$rechnungChildNodes,$sum_zapati_rechnung_array);
}

if(test_pageoverflow_noheader($pdf,40))
			pageheader($pdf,$cells_header,$rechnungChildNodes);
