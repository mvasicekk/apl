...
$pcip=get_pc_ip();
$views=array("pt_D361","pt_D361_gutstk");


$viewname=$pcip.$views[0];
$db->query("drop view $viewname");
$pt="create view $viewname";
$pt.=" as SELECT teil,auftragsnr,";
...
$pt.=" as auss_celkem ";
$pt.=" FROM `drueck` WHERE ((auftragsnr between '$auftragsnr_von' and '$auftragsnr_bis')) group by teil,auftragsnr";
...
$db->query("drop view $viewname");
$pt="create view $viewname";
$pt.=" as SELECT drueck.teil,drueck.auftragsnr,sum(`Stück`) as gut_stk";
$pt.=" FROM `drueck`";
$pt.=" join dpos on drueck.teil=dpos.teil and drueck.taetnr=dpos.`TaetNr-Aby`";
$pt.=" WHERE ((auftragsnr between '$auftragsnr_von' and '$auftragsnr_bis') and (dpos.kzgut='G'))";
...
$pt_D361=$pcip.$views[0];
$pt_D361_gutstk=$pcip.$views[1];
//$pt_D350_importgew=$pcip.$views[2];

$sql=" select teil,auftragsnr,";
...
$sql.=" gut_stk,auss_celkem";
$sql.=" from $pt_D361 join $pt_D361_gutstk using(teil,auftragsnr)";
...
function procent_auss($record)
{
	if($record['gut_stk']!=0)
		return $record['auss_celkem']/$record['gut_stk']*100;
	else
		return 0;
}
