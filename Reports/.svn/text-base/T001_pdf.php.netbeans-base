...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function zahlavi_person($pdfobjekt,$vyskaradku,$rgb,$persnr,$name,$vorname,$personChildNodes)
{
	$pdfobjekt->SetFillColor($rgb[0],$rgb[1],$rgb[2],1);
	$fill=1;
...
function zapati_vzkd($pdfobjekt,$vyskaradku,$rgb,$teilChildNodes)
{
	...
	//$obsah=number_format($obsah,0,',',' ');
	//$pdfobjekt->Cell(45,$vyskaradku,$obsah,'0',0,'R',0);
	$pdfobjekt->Cell(27,$vyskaradku,"Vzkd(P) Wettbewerb",'L',0,'L',$fill);
	...
	// celkova suma a na dalsi radek
	$pdfobjekt->SetFont("FreeSans", "", 8);
	$suma = getValueForNode($teilChildNodes,'wettkampf_celkem');
	$obsah=number_format($suma,0,',',' ');
	$pdfobjekt->Cell(0,$vyskaradku,$obsah,'1',1,'R',$fill);
	...
	// celkova suma a na dalsi radek
	$pdfobjekt->SetFont("FreeSans", "", 8);
	$suma = getValueForNode($teilChildNodes,'nowettkampf_celkem');
	$obsah=number_format($suma,0,',',' ');
	$pdfobjekt->Cell(0,$vyskaradku,$obsah,'1',1,'R',$fill);
...
$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, "T001 Fraesewettbewerb - Auswertung", $params);
//set margins
...
{
	$persnr=$person->getElementsByTagName("persnr")->item(0)->nodeValue;
	$name=$person->getElementsByTagName("Name")->item(0)->nodeValue;
	$vorname=$person->getElementsByTagName("Vorname")->item(0)->nodeValue;
		
	$personChildNodes = $person->childNodes;
	test_pageoverflow($pdf,5,$cells_header);
	zahlavi_person($pdf,5,array(255,255,200),$persnr,$name,$vorname,$personChildNodes);
	...
	foreach($teile as $teil)
	{
		...
		$factor = $teil->getElementsByTagName('fraese_wettkampf_factor')->item(0)->nodeValue;
	...
	// pokud mam nejake dily v soutezi, zobrazim vzkd pro kazdy den, vyberu treba od prvniho dilu
	// protoze tuto hodnotu mam u kazdeho dilu
	if($pocetDiluVSoutezi>0)
	{
		// vemu si prvni dil na rade
		$teilChildNodes = $teile->item(0)->childNodes;
		// radek s vzkd
		zapati_vzkd($pdf,5,array(255,255,200),$teilChildNodes);
		// zpocitat pomer casu v soutezi k celkovemu casu
		$vzkdWettbewerb = getValueForNode($teilChildNodes,"wettkampf_celkem");
		$vzkdGesamt = getValueForNode($teilChildNodes,"nowettkampf_celkem");
		if($vzkdGesamt!=0)
	...
	test_pageoverflow($pdf,5,$cells_header);
	// vypoctiPremie($itgNaFrezu,$pocetDilu,$pocetFrez,$spodniMez,$horniMez,$premiePlusKc,$premieMinusKc)
