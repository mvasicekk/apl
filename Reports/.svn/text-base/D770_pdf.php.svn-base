...
"palnr" 
=> array ("popis"=>"pal","sirka"=>10,"ram"=>'0',"align"=>"R","radek"=>0,"fill"=>1),
...
function pageheader($pdfobjekt,$cells,$childnodes)
{
	...
	$pdfobjekt->Cell(50,7,"D770",'TB',0,'L',0);
        if($teilen!=0)
...
function zapati_sestava($pdfobjekt,$vyskaradku,$rgb,$childNodes)
{
	...
	$pdfobjekt->Ln();
        $pdfobjekt->Ln();
	$pdfobjekt->Cell(0,$vyskaradku,"Cástku prosím převedte do  ".getValueForNode($childNodes,"zahlenbis")." na konto číslo :".getValueForNode($childNodes,"textkonto"),'0',1,'1',0);
