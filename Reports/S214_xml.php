...
$pt.=" as SELECT termin,dauftr.AuftragsNr,DATE_FORMAT(aufdat,'%Y-%m-%d') as aufdat, fremdauftr,Teil,`pos-pal-nr`,abgnr, kzgut,sum(`Stück`*vzkd) as sumvzkdgeplannt,sum(`Stück`*vzaby) as sumvzabygeplannt  FROM dauftr  ";
...
//$pt.=" and ((dauftr.termin between '$gepl_von' and '$gepl_bis') or dauftr.termin is null or LENGTH(TRIM(dauftr.termin))=0) and (dauftr.abgnr between '$abgnrvon' and '$abgnrbis')) GROUP BY termin,dauftr.AuftragsNr, fremdauftr,Teil,`pos-pal-nr`,abgnr";
...
$sql=" SELECT if($pt_S214_geplannt_kdminsoll.termin is null or LENGTH(TRIM($pt_S214_geplannt_kdminsoll.termin))=0,'NO TERMIN',$pt_S214_geplannt_kdminsoll.termin) as termin, $pt_S214_geplannt_ursprungstk.AuftragsNr, $pt_S214_geplannt_kdminsoll.aufdat,$pt_S214_geplannt_ursprungstk.maxoffremdauftr, ";
$sql.=" $pt_S214_geplannt_ursprungstk.Teil,$pt_S214_geplannt_kdminsoll.abgnr, $pt_S214_geplannt_kdminsoll.kzgut,";
$sql.=" dkopf.`Muster-Platz` musterplatz,dkopf.verpackungmenge,";
...
$sql.=" sum($pt_S214_geplannt_kdminsoll.sumvzkdgeplannt) as vzkd_geplant,sum($pt_S214_geplannt_kdminsoll.sumvzabygeplannt) as vzaby_geplant, Sum($pt_S214_ohneEx_minuten_aus_DRUECK.sumvzkd) AS vzkd, ";
