 ...
 * @param type $top 
 */
function printLabelHead($pdf,$vyskaRadku,$left,$top){
...
//    $wholeTableHeight = $pdf->getPageHeight()-PDF_MARGIN_BOTTOM-$top;
    $wholeTableHeight = $pdf->getPageHeight()-$top-15;
	...
	$pdf->SetX($left);
	$pdf->Cell($breiteAktual/2, $lineHeight, '', '1', 0, 'L', 0);
	$pdf->Cell($breiteAktual/2, $lineHeight, '', '1', 1, 'L', 0);
    }
}
 ...
 * @param type $childs 
 */
function print8DReport($pdf,$left,$top,$childs){
    $pdf->SetXY($left, $top);
    $width = 160;
    $height = 90;
//    $pdf->Rect($left, $top, 160, 90);
    $b1Width=90;
    $zWidth=5;
    $vWidth = 20;
    $ergWidth = 20;
...
//    $mezera = 2.5;
    ...
    $pdf->Cell($b1Width, $radek, "3. Sofortige Massnahmen", '1', 0, 'L', 0);
    $pdf->Cell($vWidth, $radek, "Verantwortl.", '1', 0, 'L', 0);
    $pdf->Cell($ergWidth, $radek, "Ergebnis", '1', 0, 'L', 0);
    $pdf->Cell($edWidth, $radek, "Einsatzdatum:", '1', 1, 'L', 0);
    $pdf->setFont("FreeSans", '', 8);
    $pdf->Cell($zWidth, $radek, "a", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3a');$pdf->Cell($b1Width-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3a_verantwortlich');$pdf->Cell($vWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3a_ergebniss');$pdf->Cell($ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3a_einsatzdatum');$pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Cell($zWidth, $radek, "b", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3b');$pdf->Cell($b1Width-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3b_verantwortlich');$pdf->Cell($vWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3b_ergebniss');$pdf->Cell($ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_3b_einsatzdatum');$pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Ln($mezera);
    
    $pdf->setFont("FreeSans", 'B', 8);
    $pdf->Cell($b1Width+$vWidth+$ergWidth, $radek, "4. Fehlerursache(n)", '1', 0, 'L', 0);
    $pdf->Cell($edWidth, $radek, "Fehleranteil[%]", '1', 1, 'L', 0);
    $pdf->setFont("FreeSans", '', 8);
    $pdf->Cell($zWidth, $radek, "a", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_4a');$pdf->Cell($b1Width+$vWidth+$ergWidth-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_4a_fehlanteil_procent');
    if($o==0) $o="";
    $pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Cell($zWidth, $radek, "b", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_4b');$pdf->Cell($b1Width+$vWidth+$ergWidth-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_4b_fehlanteil_procent');
    if($o==0) $o="";
    $pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);    
    $pdf->Ln($mezera);
    
    $pdf->setFont("FreeSans", 'B', 8);
    $pdf->Cell($b1Width, $radek, "5. Abstellmassnahme(n)", '1', 0, 'L', 0);
    $pdf->Cell($vWidth+$ergWidth, $radek, "Nachweiss", '1', 0, 'L', 0);
    $pdf->Cell($edWidth, $radek, "erw.Erfolg[%]:", '1', 1, 'L', 0);
    $pdf->setFont("FreeSans", '', 8);
    $pdf->Cell($zWidth, $radek, "a", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_5a');$pdf->Cell($b1Width-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_5a_nachweiss');$pdf->Cell($vWidth+$ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_5a_erfolg_procent');
    if($o==0) $o="";
    $pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Cell($zWidth, $radek, "b", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_5b');$pdf->Cell($b1Width-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_5b_nachweiss');$pdf->Cell($vWidth+$ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_5b_erfolg_procent');
    if($o==0) $o="";
    $pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Ln($mezera);

    $pdf->setFont("FreeSans", 'B', 8);
    $pdf->Cell($b1Width+$vWidth, $radek, "6. Eingefūhrte Massnahmen", '1', 0, 'L', 0);
    $pdf->Cell($ergWidth, $radek, "Verantwortl.", '1', 0, 'L', 0);
    $pdf->Cell($edWidth, $radek, "Einsatzdatum:", '1', 1, 'L', 0);
    $pdf->setFont("FreeSans", '', 8);
    $pdf->Cell($zWidth, $radek, "a", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_6a');$pdf->Cell($b1Width+$vWidth-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_6a_verantwortlich');$pdf->Cell($ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_6a_einsatzdatum');$pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Cell($zWidth, $radek, "b", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_6b');$pdf->Cell($b1Width+$vWidth-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_6b_verantwortlich');$pdf->Cell($ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_6b_einsatzdatum');$pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Ln($mezera);
    
    $pdf->setFont("FreeSans", 'B', 8);
    $pdf->Cell($b1Width+$vWidth, $radek, "7. Weitere Massnahmen fūr die Zukunft", '1', 0, 'L', 0);
    $pdf->Cell($ergWidth, $radek, "Verantwortl.", '1', 0, 'L', 0);
    $pdf->Cell($edWidth, $radek, "Einsatzdatum:", '1', 1, 'L', 0);
    $pdf->setFont("FreeSans", '', 8);
    $pdf->Cell($zWidth, $radek, "a", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_7a');$pdf->Cell($b1Width+$vWidth-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_7a_verantwortlich');$pdf->Cell($ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_7a_einsatzdatum');$pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
    $pdf->Cell($zWidth, $radek, "b", '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_7b');$pdf->Cell($b1Width+$vWidth-$zWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_7b_verantwortlich');$pdf->Cell($ergWidth, $radek, $o, '1', 0, 'L', 0);
    $o = getValueForNode($childs, 'report8D_7b_einsatzdatum');$pdf->Cell($edWidth, $radek, $o, '1', 1, 'L', 0);
}
/**
 *
 * @param TCPDF $pdf
...
function printAFO($pdf,$left,$top,$childs){
    ...
    // zobrazeni radku s mustrem
    // z nove tabulky s dokumentama
    $pdf->Ln(1);
    $a = AplDB::getInstance();
    ...
    $pdf->SetX($left);
    $pdf->Cell($dW+25+70-5, 5, $musterText, '0', 1, 'L', 0);
    $o =  $musterText;
...
foreach($kunden as $kunde)
{
	...
	printSchulungTable($pdf, 175, 137, 10);
//	printInterneBewertung($pdf, 110, 150, $reklChilds);
	printAFO($pdf, 170, 30, $reklChilds);
	print8DReport($pdf, PDF_MARGIN_LEFT-5, 105, $reklChilds);
    }
