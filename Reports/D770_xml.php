...
if(array_key_exists('flag_teilen', $parameters)){
    if($parameters['flag_teilen']=='1'){
        $teilen = intval($parameters['flag_teilen']);
        $abgnrVon = $parameters['abgnr_von'];
        $abgnrBis = $parameters['abgnr_bis'];
        $druckTyp = $parameters['druck_typ'];
        if($druckTyp=='ma'){
             $auftragsnrTeilen = $parameters['rechnr_ma'];
             $apl->updateMARechnr($export,$auftragsnrTeilen);
        }
        else
            $auftragsnrTeilen = $parameters['rechnr_regular'];
    }
}
...
if($teilen!=0){
    if($druckTyp=='ma'){
        $pt.=" and (drech.abgnr between '$abgnrVon' and '$abgnrBis')";
    }
    else{
        $pt.=" and not (drech.abgnr between '$abgnrVon' and '$abgnrBis')";
    }
}
$pt.=" )";
$pt.=" group by";
...
$sql.=" name2,strasse,dic,land,plz,";
