...
"betrag"
=> array ("nf"=>array(2,',',' '),"popis"=>"","sirka"=>15,"ram"=>'0',"align"=>"R","radek"=>1,"fill"=>0),
...
function zapati_teil($pdfobjekt,$vyskaradku,$rgb,$childNodes,$sumArray,$gutStk,$aussStk)
{
	...
	$pdfobjekt->SetFont("FreeSans", "B", 8);
        ...
        // pocet kusu z kterych budu pocitat cenu za dobre kusy
        $b = round($gutStk*$sumArray['preis'],2);
        $sum_zapati_rechnung_array['gutbetrag'] += $b;
        $obsah = number_format($b, 2,',',' ');
        $pdfobjekt->Cell(
                20
                ,$vyskaradku,$obsah,'T',0,'R',1);

        // pocet kusu z kterych budu pocitat cenu za zmetky
        $b = round($aussStk*$sumArray['preis']*0.7,2);
        $sum_zapati_rechnung_array['aussbetrag'] += $b;
        $obsah = number_format($b, 2, ',',' ');
        $pdfobjekt->Cell(
                20
                ,$vyskaradku,$obsah,'T',0,'R',1);
        // gesamt betrag
        $b = round($gutStk*$sumArray['preis'],2)+round($aussStk*$sumArray['preis']*0.7,2);
        $sum_zapati_rechnung_array['gesbetrag'] += $b;
        $obsah = number_format($b, 2, ',',' ');
        $pdfobjekt->Cell(
                20
                ,$vyskaradku,$obsah,'T',0,'R',1);
            ...
            foreach ($teile as $teil) {
                ...
                foreach ($taetigkeiten as $taetigkeit) {
                    ...
                    foreach ($kusy as $kus) {
                        ...
                        if(intval(getValueForNode($kusChildNodes, 'stk'))<$teilGutStk) $teilGutStk = intval(getValueForNode($kusChildNodes, 'stk'));
