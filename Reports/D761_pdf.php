...
function pageheader($pdfobjekt,$cells,$childnodes)
{
	...
	$pdfobjekt->Cell(60,5,"$vomZeile",'0',0,'L',0);
	...
	if($pdfobjekt->PageNo()==1)
	{
		...
		$pdfobjekt->Cell(0,7,getValueForNode($childnodes,"rechtext"),'0',1,'L',0);
	}
	else
	{
		$pdfobjekt->SetFont("FreeSans", "B", 8);
...
function zapati_teil($pdfobjekt,$vyskaradku,$rgb,$childNodes,$sumarray,$w)
{

	$pdfobjekt->SetFillColor($rgb[0],$rgb[1],$rgb[2],1);
	$pdfobjekt->SetFont("FreeSans", "B", 8);
	$pdfobjekt->Cell(20+45+45+12+12+20,$vyskaradku,"Summe Teil",'B',0,'L',1);
...
function zapati_rechnung($pdfobjekt,$vyskaradku,$rgb,$childNodes,$sumarray)
{
	...
	$pdfobjekt->SetFillColor($rgb[0],$rgb[1],$rgb[2],1);
	$pdfobjekt->SetFont("FreeSans", "B", 8);
	$pdfobjekt->Cell(20+45+45+12+12,$vyskaradku,"",'T',0,'R',1);
...
function zapati_sestava($pdfobjekt,$vyskaradku,$rgb,$childNodes)
{
	...
	//$dic=getValueForNode($childNodes,"vomdic");
...
//$pdf->SetHeaderData("", 0, "D761 Rechnung", $params);
$pdf->SetHeaderData("", 0, "", "");
			...
			foreach($teile as $teil)
			{
				...
				foreach($taetigkeiten as $taetigkeit)
				{
                    ...
                    foreach($kusy as $kus){
                        ...
                        //aktualizuju sumy pro zapati teil
...
//					}
			...
			if(test_pageoverflow_noheader($pdf,5))
						pageheader($pdf,$cells_header,$rechnungChildNodes);
			zapati_teil($pdf,5,array(255,255,255),$fremdpositionChildNodes,$sum_zapati_teil_array,$wahr);
			}
		//}
	//}
	// zapati rechnung
