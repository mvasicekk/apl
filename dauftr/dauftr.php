...
$smarty = new Smarty;
	...
	{
		$sql="select AuftragsNr as auftragsnr,kunde,minpreis,bestellnr,bemerkung,DATE_FORMAT(Aufdat,'%d.%m.%Y') as aufdat,DATE_FORMAT(fertig,'%d.%m.%Y') as fertig,DATE_FORMAT(ausliefer_datum,'%d.%m.%Y') as ausliefer_datum,DATE_FORMAT(ex_datum_soll,'%d.%m.%Y %H:%i') as ex_datum_soll from daufkopf where (auftragsnr='".$_GET['auftragsnr']."')";
		$res=mysql_query($sql);
