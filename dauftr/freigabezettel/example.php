...
//    $res = mysql_query("select * from dtattypen");
	...
	if(isset($_GET['lastdatum'])) 
        ...
        $sql = "select id,`PersNr` as persnr,DATE_FORMAT(`Datum`,'%d.%m.%Y') as datum,FORMAT(`Stunden`,2) as stunden,`Schicht` as schicht,tat as oe, DATE_FORMAT(anw_von,'%H:%i') as von,DATE_FORMAT(anw_bis,'%H:%i') as bis,FORMAT(pause1,2) as pause1,FORMAT(pause2,2) as pause2,comp_user_accessuser as user,stamp from dzeit order by stamp desc limit 10";
        $res = mysql_query($sql);
        $items = array();
