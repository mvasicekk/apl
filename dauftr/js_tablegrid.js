...
function save(control)
{
	...
	// jeste budu potrebovat i cislo zakazky pro eventuelni upravu vzkd v druecku
	parametry+="&auftragsnr="+document.getElementById('auftragsnr').value;
	
	//okno=window.open();
	//okno.document.write(parametry);
...
function edit(xml)
{
	...
	e_delete.setAttribute('onclick','deletedauftr(this);');
	...
	//alert(bunky_array.item(promenne.length).firstChild.data);
	aNode = document.getElementById(radekid);
	//alert(aNode);	
	bunky_array.item(promenne.length).replaceChild(list_item_save,aNode);
...
function deletedauftr(control)
{
	...
	if(odpoved)
	{
		...
		// musim najit input, kterej se jmenu KzGut
		var inputy = radek.getElementsByTagName('input');
		//alert('inputy='+inputy);
		for(j=0;j<inputy.length;j++)
		{
			// zjistim si id inputu
			var input = inputy.item(j);
			//alert('input='+input);
			var idinputu = input.getAttribute('id');
			//alert('idinputu='+idinputu);
			if(idinputu=="KzGut")
			{
				// pokud mam input kzgut ulozim si jeho hodnotu
				kzgut = input.getAttribute('value');
				//if(input.hasChildNodes())
				//	kzgut = input.firstChild.nodeValue;
			}	
		}
		...
		//parametry=parametry.substring(0,parametry.length-1);
		parametry = '';
		parametry+="id="+control.id.substring(4,control.id.length)+"&kzgut="+kzgut; 
		parametry=encodeURI(parametry);
		//alert('parametry = '+parametry);
		// pomoci AJAX updatnu radek v Dauftr
		getDataReturnXml('./delete_dauftr.php?'+parametry, update_dauftr);
		return;
	}
	else
		return;
