...
function stahniDataMinuty(auftr, teil, tat, akce) {
if (tat!=0){
  var hlavicka = document.getElementsByTagName("head")[0];
  var dataLoader = document.getElementById("scriptLoader");
  ...
  script = document.createElement("script");
  script.id = "scriptLoader";
  script.src = "./operace.php?auftr="+auftr+"&teil="+teil+"&tat="+tat+"&akce="+akce;
  var x = document.getElementsByTagName("head")[0];
  x.appendChild(script);
  }else{
...
function naplnOperace(pole){
  ...
  document.getElementById('tatigkeit').options.length = 0;
    for (i=0; i<(operace.length);i++){
      if (document.all) {
        var arg1=document.getElementById('tatigkeit').length;
      }else{
        var arg1=0
      }
      ...
      no.value = operace[i];
      no.text = operace[i];
      document.getElementById('tatigkeit').add(no,arg1)
    }
...
function naplnMin(VzAby, VzKz, Bez){
document.getElementById('tatzeit').value=VzAby;
document.getElementById('kz').value=VzKz;
document.getElementById('beschreibung').value=Bez;
}
...
//Control Function
  ...
  function control(){
    ...
    var taetNr = document.getElementById('tatigkeit').value; 
                     
    var taetAbyZeit = document.getElementById('tatzeit').value;

    var taetKdZeit = document.getElementById('kz').value;
